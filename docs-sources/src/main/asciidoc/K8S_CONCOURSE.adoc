...
To get the contents of CA for GCE just type
...
and you will get sth like this:
...
- cluster:
    ...
    server: https://ipOfTheServer
  name: someName
----

To get the CA you have to copy the contents of `certificate-authority-data` to
a file and decode it from base64
...
Set that value under `paas-test-ca`, `paas-stage-ca` and `paas-stage-ca`

To get the token just type:

[source,bash]
----
$ kubectl exec -it $( kubectl get pods | grep concourse-web | awk '{print $1}' ) -- cat /var/run/secrets/kubernetes.io/serviceaccount/token
----
