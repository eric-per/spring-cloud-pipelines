...
|PAAS_TEST_CLIENT_TOKEN_ID | Access token for test environment |
|PAAS_STAGE_CLIENT_TOKEN_ID | Access token for test environment |
|PAAS_PROD_CLIENT_TOKEN_ID | Access token for test environment |
...
// TODO: Improve this, currently it's just a set of notes
=== Connecting to a cluster @ PKS

- Log in to the cluster
- To see the dashboard just do `kubectl proxy` and access `localhost:8081/ui`
- Deploy Jenkins to the cluster (`deploy-helper.sh`)
- Deploy Artifactory to the cluster (`deploy-helper.sh`)
- Update `git` credential
- Update `settings.xml` and `gradle.properties` with a script
```groovy
...
println "Done!"
```
- Store the certificate manually on Jenkins master
```
new File("/root/.kubernetes/").mkdirs()
new File("/root/.kubernetes/ca.crt").text="""
-----BEGIN CERTIFICATE-----
...
-----END CERTIFICATE-----
"""
